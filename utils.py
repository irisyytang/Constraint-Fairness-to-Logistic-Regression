...
def compute_cross_validation_error(x_all, y_all, x_control_all, num_folds, loss_function, apply_fairness_constraints, apply_accuracy_constraint, sep_constraint, sensitive_attrs, sensitive_attrs_to_cov_thresh_arr, gamma=None):
    ...
    def train_test_single_fold(train_data, test_data, fold_num, output_folds, sensitive_attrs_to_cov_thresh):
        ...
        output_folds.append([fold_num, test_score, train_score, correlation_dict_test, correlation_dict_train, cov_dict_test, cov_dict_train])
    ...
    for i in range(0,num_folds):
        ...
        train_test_single_fold(train_folds[i], test_folds[i], i, results, sensitive_attrs_to_cov_thresh_arr[i])
    
    """ Track the training time of different experiment group for training complexity comparison"""
    finish = time.time()
    duration = finish - start
    print("10 fold training time:", duration)
        ...
        for feature_val, feature_dict in v.items():
            for class_label, frac_class_arr in feature_dict.items():
                ...
                try:
                    ...
                    print(k, feature_val, class_label, frac_class_arr)
                    print(correlation_dict_avg)

    return correlation_dict_avg



def plot_cov_thresh_vs_acc_pos_ratio(x_all, y_all, x_control_all, num_folds, loss_function, apply_fairness_constraints, apply_accuracy_constraint, sep_constraint, sensitive_attrs, img_id):
    ...
    test_acc_arr, train_acc_arr, correlation_dict_test_arr, correlation_dict_train_arr, cov_dict_test_arr, cov_dict_train_arr = compute_cross_validation_error(x_all, y_all, x_control_all, 
                                                                           num_folds, loss_function, 0, apply_accuracy_constraint, 
                                                                           sep_constraint, sensitive_attrs, [{} for i in range(0,num_folds)], 0)

    for c in cov_range:
        print("Covariance value is:", c)
    ...
    plt.savefig("img/demo_{0}.png".format(img_id))
    plt.clf()
